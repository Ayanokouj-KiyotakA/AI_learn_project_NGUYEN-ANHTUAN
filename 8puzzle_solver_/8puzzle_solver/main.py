...
class PuzzleApp:
    ...
    def _build_ui(self):
        ...
        algo_menu = ttk.Combobox(algo_frame, textvariable=self.algo_var,
                                 values=["BFS", "DFS", "DLS", "IDS", "UCS",
                                         "Greedy", "A*", "IDA*",
                                         "Hill Climbing", "Stochastic HC",
                                         "Random Restart HC", "Local Beam"],
                                 width=16, state="readonly", font=("Courier", 11))
        ...
        # Beam k (chi hien thi khi chon Local Beam)
        self.beam_frame = tk.Frame(left, bg=COLORS["bg"])
        tk.Label(self.beam_frame, text="Beam k:", font=("Courier", 10),
                 bg=COLORS["bg"], fg=COLORS["text"]).pack(side=tk.LEFT)
        tk.Spinbox(self.beam_frame, from_=1, to=20, textvariable=self.beam_k,
                   width=5, font=("Courier", 10)).pack(side=tk.LEFT, padx=6)
    ...
    def _on_algo_change(self, event=None):
        ...
        if self.algo_var.get() == "Local Beam":
            self.beam_frame.pack(fill=tk.X, pady=2)
        else:
    ...
    def _on_solve_done(self, result, nodes, elapsed, algo):
        ...
        if result is None:
            ...
            local_algos = ["Hill Climbing", "Stochastic HC", "Random Restart HC", "Local Beam"]
            if algo in local_algos:
                self._log("Bi ket tai cuc bo (local optimum)!", "highlight")
                self._log(f"{algo} khong thoat duoc.", "highlight")
            else:
